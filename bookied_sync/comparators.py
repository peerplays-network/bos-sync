...
def cmp_dynamic_bmg_fuzzy(spread=1):
    ...
    def cmp(soll, ist):
        ...
        if self_description["_dynamic"].lower() == "hc":
            assert (
                "_hca" in self_description
                and "_hch" in self_description
                and "_hca" in description
                and "_hch" in description
            ), "dynamic betting market is missing _hca or _hch"

            # Need the handicap of home and away to match fuzzy
            if in_range(description["_hch"], self_description["_hch"]) and in_range(
                description["_hca"], self_description["_hca"]
            ):
                return True
        ...
        # Overunder #########################
            ...
            assert (
                "_ou" in self_description and "_ou" in description
            ), "dynamic betting market is missing _overunder"
            return in_range(description["_ou"], self_description["_ou"])
...
def cmp_langs(key, langs=None):
    ...
    def cmp(soll, ist, langs=langs, key=key):
        ...
        return all(
            [bool([k, dList2Dict(soll.description).get(k)] in ist) for k in langs]
        )
...
def cmp_all_langs(key):
    ...
    def cmp(soll, ist):
        ...
        _ist = ist.get(key, ist.get("new_{}".format(key)))
        return (
            (bool(_ist) and bool(_soll))
            and all([a in _ist for a in _soll])
            and all([b in _soll for b in _ist])
        )
...
def cmp_status():
    ...
    def cmp(soll, ist):
        return (
            not bool(soll.get("status"))
            or not bool(ist.get("status"))
            or ist.get("status") == soll.get("status")
        )
...
def cmp_start_time():
    ...
    def cmp(soll, ist):
        ...
        return (
            not bool(soll.get("start_time"))
            or not bool(ist.get("start_time"))
            or ist.get("start_time") == formatTime(soll.get("start_time"))
        )
